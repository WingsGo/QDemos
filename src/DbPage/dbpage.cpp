...
void DbPage::bindData(const QString &sql)
{
    ...
    if (labPageCurrent != 0) {
        labPageCurrent->setText(QString::fromLocal8Bit("第 %1 页").arg(pageCurrent));
    }

    if (labPageCount != 0) {
        labPageCount->setText(QString::fromLocal8Bit("共 %1 页").arg(pageCount));
    }

    if (labResultCount != 0) {
        labResultCount->setText(QString::fromLocal8Bit("共 %1 条").arg(resultCount));
    }

    if (labResultCurrent != 0) {
        labResultCurrent->setText(QString::fromLocal8Bit("每页 %1 条").arg(resultCurrent));
    }

    if (labInfo != 0) {
        labInfo->setText(QString::fromLocal8Bit("共 %1 条  每页 %2 条  共 %3 页  第 %4 页").arg(resultCount).arg(resultCurrent).arg(pageCount).arg(pageCurrent));
    }
...
{
    if (labResult != 0) {
        labResult->setText(QString::fromLocal8Bit("查询用时 %1 秒").arg(QString::number(msec / 1000, 'f', 3)));
    }
...
void DbPage::select()
{
    ...
    QString info = QString::fromLocal8Bit("正在查询...");
